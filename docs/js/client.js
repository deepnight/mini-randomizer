...
var App = function() {
	...
	if(this.settings.curFileId != null) {
		if(this.getFile(this.settings.curFileId) == null) {
...
App.prototype = $extend(dn_Process.prototype,{
	...
	,getAllFileIds: function() {
		...
		var all = [];
		var h = this.internalFiles.h;
		var fid_h = h;
		var fid_keys = Object.keys(h);
		var fid_length = fid_keys.length;
		var fid_current = 0;
		while(fid_current < fid_length) {
			var fid = fid_keys[fid_current++];
			if(!Object.prototype.hasOwnProperty.call(dones_h,fid)) {
				all.push(fid);
			}
		}
		var _g = 0;
...
var EditorUI = function() {
	...
	this.ace.commands.addCommand({ bindKey : { win : "F3"}, exec : function(_) {
		_gthis.ace.execCommand("find");
	}});
	this.ace.commands.addCommand({ name : "Duplicate", bindKey : { win : "ctrl-D"}, exec : function(_) {
		_gthis.ace.execCommand("duplicateSelection");
	}});
	this.ace.commands.addCommand({ name : "Remove line", bindKey : { win : "ctrl-shift-D"}, exec : function(_) {
		_gthis.ace.execCommand("removeline");
	}});
	this.ace.focus();
...
EditorUI.prototype = $extend(SiteProcess.prototype,{
	addLineMark: function(line,className) {
		this.ace.session.addMarker(new ace.Range(line - 1,1,line - 1,1000),"cust-" + className,"fullLine");
	}
	,addLog: function(str,line,className) {
		var _gthis = this;
		var jLine = $("<pre>Line " + line + " -- <strong>" + str + "</strong></pre>");
		jLine.addClass(className);
		jLine.click(function(_) {
			App.ME.openEditor();
			App.ME.editor.gotoLine(line);
		});
		this.jLog.append(jLine);
	}
	,addErrors: function(errors) {
		...
		while(_g < errors.length) {
			var e = errors[_g];
			++_g;
			this.addLineMark(e.line,"error");
		}
		var _g = 0;
		while(_g < errors.length) {
			var e = errors[_g];
			++_g;
			this.addLog(e.err,e.line,"error");
			this.jLog.addClass("errors");
		}
	}
	,clearLog: function() {
		this.jLog.removeClass("errors");
		this.jLog.empty();
		this.ace.session.clearAnnotations();
		var markers = this.ace.session.getMarkers();
		var _g = 0;
		var _g1 = Reflect.fields(markers);
		while(_g < _g1.length) {
			var k = _g1[_g];
			++_g;
			var m = Reflect.field(markers,k);
			if(m.clazz.indexOf("cust-") == 0) {
				this.ace.session.removeMarker(m.id);
			}
		}
	...
	,update: function() {
		...
		if(this.invalidatedMapMarkers && !this.cd.fastCheck.h.hasOwnProperty(20971520)) {
			this.updateMapMarkers();
		}
	}
	,__class__: EditorUI
...
RandomParser.run = function(raw) {
	...
	var lines = raw.split("\n");
	var curKey = null;
	var optionReg = new EReg(RandomParser.OPTION_REG,"");
	...
	while(_g < lines.length) {
		...
		if(optionReg.match(l)) {
			var o = RandomParser.parseOption(l,lineIdx1,_err);
			if(o != null) {
				rdata.options.push(o);
				continue;
			} else {
				_err("Cannot parse this option",lineIdx1);
			}
		}
		if(keyDefinionReg.match(l)) {
			...
			if(probaMulReg.match(l)) {
				probaMul = parseFloat(probaMulReg.matched(1));
		...
		while(_g < _g1.length) {
			...
			while(keyRefReg.match(tmp)) {
				...
				if(!countReg.match(k) && !Object.prototype.hasOwnProperty.call(rdata.tables.h,k)) {
					_err("Unknown key \"@" + k + "\" in \">" + table_key + "\"",e.line);
				}
	...
	while(_g < _g1.length) {
		...
		++_g;
		if(o.id == "button") {
			rdata.markedLines.push({ parentKey : o.args.h["key"], line : o.line, className : "button"});
			var k = o.args.h["key"];
	...
	if(o == "button") {
		...
		if(rawArgs == null) {
			...
			if(probaMulReg.match(rawArgs)) {
				var value = probaMulReg.matched(1);
				args.h["count"] = value;
			...
			if(label.length == 0 && Object.prototype.hasOwnProperty.call(args.h,"key")) {
				label = args.h["key"];
			}
			args.h["label"] = label;
		}
	} else {
		onError("Unknown option: #" + o,lineIdx);
	}
	return { id : o, line : lineIdx, args : args};
};
...
RandomParser.removeSpecialChars = function(str) {
	var r_r = new RegExp("[^a-z0-9-_.:,;!?'\"]","gim".split("u").join(""));
	return str.replace(r_r," ");
};
var RandomUI = function() {
	var _gthis = this;
	SiteProcess.call(this,"random");
	this.jToolbar = this.jRoot.find(".toolbar");
	this.jRandButtons = this.jToolbar.find(".randButtons");
	this.jOutput = this.jRoot.find(".output");
	this.jToolbar.find(".clear").click(function(_) {
		_gthis.clearOutput();
	});
};
$hxClasses["RandomUI"] = RandomUI;
...
RandomUI.prototype = $extend(SiteProcess.prototype,{
			...
			if(o[0].id == "button") {
				...
				jBt.click((function(o) {
					return function(ev) {
						...
						var count = Std.parseInt(o[0].args.h["count"]);
						if(ev.shiftKey) {
							_gthis.clearOutput();
							count = 10;
						}
						...
						while(_g < _g1) {
							var i = _g++;
							_gthis.output(_gthis.randomizer.draw(o[0].args.h["key"]));
						}
					};
				})(o));
				this.jRandButtons.append(jBt);
			} else {
				App.ME.notify("Unknown option: " + o[0].id);
			}
		}
		this.clearOutput();
	}
	...
	,output: function(str) {
		...
		str = "<pre>" + str.split("\\n").join("</pre><pre>") + "</pre>";
		this.jOutput.prepend("<div class=\"entry\">" + str + "</div>");
	}
	,onDispose: function() {
		SiteProcess.prototype.onDispose.call(this);
	}
...
Randomizer.prototype = {
	draw: function(key) {
		return this.drawRec(key,new haxe_ds_IntMap());
	}
	,drawRec: function(key,alreadyDones) {
		...
		while(_g < table.length) {
			var e = table[_g];
			++_g;
			if(!alreadyDones.h.hasOwnProperty(e.line)) {
		...
		alreadyDones.h[entry.line] = true;
		var out = entry.raw;
		...
		while(quickListReg.match(out)) {
			...
			var e = StringTools.trim(all.length == 0 ? null : all[Std.random(all.length)]);
		...
		while(countReg.match(out)) {
			...
			out = countReg.matchedLeft() + (min + Std.random(max - min + 1)) + countReg.matchedRight();
		}
		var refReg = new EReg(RandomParser.KEY_REFERENCE_REG,"");
		while(refReg.match(out)) {
			var k = refReg.matched(1);
			out = refReg.matchedLeft() + this.drawRec(k,alreadyDones) + refReg.matchedRight();
		}
		if(out == "-") {
			out = "";
		}
...
dn_data_LocalStorage.createJsonStorage = function(name,prettyLevel) {
	if(prettyLevel == null) {
...
dn_Cooldown.__meta__ = { obj : { indexes : ["test","jump","a","b","c","mapLock"]}};
